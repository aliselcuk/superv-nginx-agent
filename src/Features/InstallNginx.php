    ...
    public function handle(StubBuilder $builder)
    {
        ...
        $this->job('Add DotDeb Sources',
            $builder->wrapper('save_to_file', ['target' => '/etc/apt/sources.list.d/dotdeb.list'])
                    ->build('superv.agents.nginx::dotdeb_sources', ['distro' => 'stretch'])
        );
...
//        $this->server->saveToFile($this->dispatch(new MakeStub('superv::installer.supervisor', ['home_dir' => '/usr/local/superv/app'])), '/etc/supervisor/conf.d/superv.conf');
//
//
...
//        $this->job('Create SuperV DB User '.$mysqlAppPass, "mysql -u root -e \"CREATE USER 'superv_app'@'localhost' IDENTIFIED BY '{$mysqlAppPass}';\"");
//        $this->job('Grant DB PRIVILEGES', "mysql -u root -e \"GRANT ALL ON superv_app.* TO 'superv_app'@'localhost'; FLUSH PRIVILEGES;\"");
        ...
        $this->job('Get SuperV', 'rm -Rf /usr/local/superv/app; composer create-project superv/superv /usr/local/superv/app dev-master');

        $mysqlAppPass = 'A1JVjUjQbe9MT7o4gOBC/Q==';
